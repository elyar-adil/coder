...
- Click a disclosure arrow to expand Thinking, scroll with the wheel, or drag across visible conversation text to select it. `Ctrl+C` or right-click copies a selection; without a selection Ctrl+C exits. `Escape` clears the selection. Typing still edits the draft.
...
Decorative animation is disabled by default so local, Windows, and SSH sessions
share the same quiet rendering. Set `MAW_MOTION=full` (or `CODER_MOTION=on`) to
opt back into the animated welcome mark, spinner, and smooth scrollbar.
