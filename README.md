...
- `!<command>` runs a shell command directly in the current workspace, outside the agent loop: output streams into a scrollable popup, `Ctrl+C` stops the command, and nothing is sent to the model or recorded in the conversation. `/`-commands stay reserved for the built-in slash commands.
