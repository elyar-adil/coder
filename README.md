...
- `!<command>` runs a shell command directly in the current workspace, outside the agent loop: the command and its output stream inline into the conversation transcript (never sent to the model or fed back into context), and `Ctrl+C` stops a running command. `/`-commands stay reserved for the built-in slash commands.
