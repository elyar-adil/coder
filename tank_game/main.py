...
class Tank:
    ...
    def update_bullets(self):
        for bullet in self.bullets[:]:
            ...
            # 移除出界子弹
            if not pygame.Rect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT).colliderect(bullet):
    ...
    def draw(self, screen):
        ...
        if self.direction == 'UP':
            pygame.draw.line(screen, WHITE, center, (center[0], center[1] - TANK_SIZE//2), 3)
        elif self.direction == 'DOWN':
            pygame.draw.line(screen, WHITE, center, (center[0], center[1] + TANK_SIZE//2), 3)
        elif self.direction == 'LEFT':
            pygame.draw.line(screen, WHITE, center, (center[0] - TANK_SIZE//2, center[1]), 3)
        elif self.direction == 'RIGHT':
            pygame.draw.line(screen, WHITE, center, (center[0] + TANK_SIZE//2, center[1]), 3)
...
class EnemyTank(Tank):
    ...
    def update(self):
        ...
        if self.direction == 'UP': self.move(0, -ENEMY_SPEED)
        elif self.direction == 'DOWN': self.move(0, ENEMY_SPEED)
        elif self.direction == 'LEFT': self.move(-ENEMY_SPEED, 0)
        elif self.direction == 'RIGHT': self.move(ENEMY_SPEED, 0)
...
def main():
    ...
    enemies = [EnemyTank(random.randint(0, SCREEN_WIDTH-TANK_SIZE), 
                        random.randint(0, SCREEN_HEIGHT//3)) for _ in range(3)]
        ...
        for enemy in enemies[:]:
            ...
            for bullet in player.bullets[:]:
                if enemy.rect.colliderect(bullet):
                    ...
                    score += 1
                    # 产生新敌人
                    enemies.append(EnemyTank(random.randint(0, SCREEN_WIDTH-TANK_SIZE), 
